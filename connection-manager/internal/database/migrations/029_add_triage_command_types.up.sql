-- 029_add_triage_command_types.up.sql
-- Add post-isolation triage command types to the commands_command_type_check constraint.
-- Migration 024 created the playbook pipeline tables but forgot to expand the CHECK constraint.
-- The models.go API validator already accepts these types; this aligns the DB constraint.
...
ALTER TABLE commands
    ADD CONSTRAINT commands_command_type_check CHECK (command_type IN (
        ...
        -- Post-isolation triage commands (added by 024 but missing from constraint)
